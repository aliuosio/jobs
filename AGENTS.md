...
AI-powered job application management system. Four components: FastAPI backend (RAG pipeline + form filling), Firefox extension (Job Forms Helper + Job Links Manager), n8n automation workflows, and comprehensive test suite.
...
│   ├── services/          # Core business logic (retriever, generator, etc.)
...
| RAG Pipeline | `src/services/retriever.py`, `embedder.py`, `generator.py` | Hybrid search + HyDE + reranking |
...
| ------ | ---- | -------- | ---- | ---- |
| app | FastAPI | src/main.py | 5 | Entry point |
| router | APIRouter | src/api/routes.py | 3 | HTTP surface |
| Retriever | Service | src/services/retriever.py | 4 | Vector search |
| QueryCache | Utility | src/utils/cache.py | 3 | HyDE caching |

## CONVENTIONS

- Python: `src` layout, `__init__.py` per package, Pydantic v2
- Ruff linter (line-length=100, target-version=py311)
- No explicit pyproject.toml - uses requirements.txt in `.docker/api-backend/`
- Browser extension: ES Module conversion in progress (see REFACTORING-PLAN.md)
...
- **Docker-First**: Always use Docker for backend operations (run, test, etc.)
- **No Unnecessary Comments**: Only essential docstrings allowed (public APIs, complex logic)
- **Code Quality**: No type suppression, no empty catch blocks
...
- FastAPI with lifespan context (Qdrant + PostgreSQL connection)
- HyDE (Hypothetical Document Embeddings) for retrieval improvement
- SSE broadcasting for real-time job offers updates
- Extension uses signal-extractor for field classification
...
- Browser storage.local (extension) (005-copy-cover-letter)

- Python 3.11 (backend), JavaScript (extension) + FastAPI, PostgreSQL, Qdrant (backend); extension services (003-cover-letter-issues)
- PostgreSQL (`job_applications` table) (003-cover-letter-issues)

## Recent Changes

- 003-cover-letter-issues: Added Python 3.11 (backend), JavaScript (extension) + FastAPI, PostgreSQL, Qdrant (backend); extension services
